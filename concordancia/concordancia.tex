...
La concordancia entre mediciones es de gran interés en la práctica clínica. Las técnicas de análisis de la concordancia dependen del tipo de
...
El índice estadístico que permite cuantificar el acuerdo entre variables numéricas es el \emph{Coeficiente de Correlación Intraclase
...
\subsection{Concordancia entre dos variables cualitativas: Kappa de Cohen}
...
El ejemplo anterior pone de manifiesto la discrepancia tan grande que puede haber entre la proporción o porcentaje de observaciones
...
\begin{indicacion}
\begin{enumerate}
\item Seleccionar el menú \menu{Analizar\flecha Escala\flecha Análisis de fiabilidad}.
\item Seleccionar las variables \textsf{tens\_habitual} y \textsf{tens\_muñeca} al campo \campo{Elementos}, y hacer click en el botón
\texttt{Estadísticos}.
\item En el cuadro de diálogo resultante marcar \texttt{Coeficiente de correlación intraclase}.
\item En el campo \campo{Modelo:} seleccionar la opción \opcion{Dos factores, efectos mixtos} y en el campo \campo{Tipo:} seleccionar la opción \opcion{Acuerdo absoluto}. Se ha escogido el modelo indicado pues el primer factor de variabilidad serían los pacientes, que entendemos que se han escogido de forma aleatoria y sus efectos serían aleatorios, y el segundo factor es el aparato de medida que en nuestro caso es de efectos fijos, pues el tensiómetro habitual y el de muñeca no se han sacado de una muestra aleatoria de tensiómetros.
\item Hacer click sobre el botón \boton{Continuar} y \texttt{Aceptar}.
%\item El Coeficiente de correlación intraclase es $0.929$ lo que indica que el nivel de concordancia entre las medidas de la presión
...
\begin{indicacion}
\begin{enumerate}
\item Seleccionar el menú \menu{Gráficos\flecha Cuadros de diálogo antiguos\flecha Dispersión/Puntos}, elegir la opción \opcion{Dispersión simple} y hacer click sobre el botón \boton{Definir}.
\item Seleccionar la variable \variable{tens\_muñeca} en el campo Eje Y del cuadro de diálogo.
\item Seleccionar la variable \variable{tens\_habitual} en el campo Eje X del cuadro de diálogo y hacer click sobre el botón \boton{Aceptar}.
...
\item Cerrar la ventana \textt{Propiedades} y la ventana del editor de gráficos.
\end{enumerate}
\end{indicacion}
...
\item Hacer click con el botón derecho del ratón y seleccionar \texttt{Añadir línea de referencia desde la ecuación} y en el campo \campo{Ecuación personalizada} que aparece en \texttt{Línea de referencia} de la ventana \texttt{Propiedades}, escribir x para representar la recta y = x.
\item Cerrar la ventana \textt{Propiedades} y la ventana del editor de gráficos.
\end{enumerate}
\end{indicacion}

\item Crear la variable \variable{diferencia} obtenida restando \textsf{tens\_habitual} menos  \textsf{tens\_muñeca}
\begin{indicacion}
\begin{enumerate}
\item Seleccionar el menú \menu{Transformar\flecha Calcular...}
\item Introducir el nombre de la nueva variable que vamos a crear, en este caso \textsf{diferencia}, dentro del campo \campo{Variable de destino:}.
...
\item Crear la variable \variable{tens\_media} cuyos valores serán las medias de \textsf{tens\_habitual} y  \textsf{tens\_muñeca}
\begin{indicacion}
\begin{enumerate}

\item Seleccionar el menú \menu{Transformar\flecha Calcular...}
\item Introducir el nombre de la nueva variable que vamos a crear, en este caso \textsf{tens\_media}, dentro del campo \campo{Variable de
destino:}.
...
\begin{enumerate}
\item Seleccionar el menú \menu{Gráficos\flecha Cuadros de diálogo antiguos\flecha Dispersión/Puntos}, elegir la opción \opcion{Dispersión simple} y hacer click sobre el botón \boton{Definir}.
\item Seleccionar la variable \variable{diferencia} en el campo Eje Y del cuadro de diálogo.
\item Seleccionar la variable \variable{tens\_media} en el campo Eje X del cuadro de diálogo y hacer click sobre el botón \boton{Aceptar}.
\end{enumerate}
\end{indicacion}

\item Crear la variable \variable{dif\_abs}, cuyos valores serán los valores absolutos de la variable \variable{diferencia}, y la variable
\textsf{desacuerdo}, que tomará el valor $0$ si las medidas con ambos métodos son iguales y el valor $1$ si son distintas.

\begin{indicacion}
\begin{enumerate}
\item Seleccionar el menú \menu{Transformar\flecha Calcular...}
\item Introducir el nombre de la nueva variable que vamos a crear, en este caso \textsf{dif\_abs}, dentro del campo \campo{Variable de
destino:}.
\item En el cuadro de diálogo \texttt{Expresión numérica:} escribir ABS(\textsf{diferencia}) y hacer click sobre el botón \boton{Aceptar}.
\item Crear una variable denominada \textsf{desacuerdo} e introducir un $0$ cuando  \textsf{dif\_abs} sea $0$ y un $1$ en caso contrario.
\end{enumerate}
\end{indicacion}

\item Representar gráficamente el porcentaje de discordancia entre ambos métodos de medición de la tensión arterial.
\begin{indicacion}
\begin{enumerate}
\item Seleccionar el menú \menu{Analizar\flecha Superviv.\flecha Kaplan-Meier}.
\item Seleccionar la variable \variable{dif\_abs} al campo \campo{Hora:} y la variable \variable{desacuerdo} al campo \campo{Estado:}.
\item Hacer click en el botón \boton{Definir evento...}.
\item En el cuadro de diálogo resultante seleccionar la opción \opcion{Valor único:}, poner un $1$ en dicho campo y hacer click sobre el botón \boton{Continuar}.
\item Hacer click sobre el botón \boton{Opciones} y marcar \texttt{Suprviv.} en Gráficos.
\item Hacer click sobre el botón \boton{Continuar} y \texttt{Aceptar}.
\end{enumerate}
\end{indicacion}
\end{enumerate}
...
\item Se entregaron a dos radiólogos A y B un conjunto de radiografías de tórax de pacientes oncológicos, para que informaran si presentaban
metástasis en los pulmones o no. Ambos radiólogos analizaron todas las radiografías y cada uno de ellos emitió su informe, indicando en
cuáles de ellas se apreciaban metástasis y en cuáles no. Como resultado de dichos informes hubo 32 radiografías en que ambos radiólogos
apreciaron metástasis, 68 radiografías en que ninguno las apreció, 6 radiografías en que el radiólogo A las apreció y el B no, y 10 en que
el B las apreció y el A no.
...
mediante}, seleccionar la variable \variable{frecuencia} en el campo \campo{Variable de frecuencia} y hacer click en el botón \boton{Aceptar}.
\end{enumerate}
\end{indicacion}
...
contingencia}.
\item En el cuadro de diálogo que aparece, seleccionar la variable \variable{opiniónA} al campo \campo{Filas} y la variable \texttt{opiniónB} al campo \campo{Columnas}, y hacer click sobre el botón \boton{Estadísticos}.
\item En el cuadro de diálogo que aparece, marcar la casilla \texttt{Kappa}, y hacer click en el botón \boton{Continuar} y \texttt{Aceptar}.
%\item El valor del índice de Kappa obtenido es $0.695$ por lo que la concordancia es buena.
\end{enumerate}
\end{indicacion}
\end{enumerate}
...
\begin{indicacion}
\begin{enumerate}
\item Seleccionar el menú \menu{Analizar\flecha Pruebas no paramétricas\flecha Cuadros de diálogo antiguos\flecha K muestras relacionadas}.
...
\texttt{Variables de contraste}, marcar en  \texttt{Tipo de prueba} la opción \opcion{W de Kendall} y hacer click en el botón \boton{Aceptar}.
